...
dotnet publish (Join-Path $repositoryRoot 'src\DeskBridge.Host\DeskBridge.Host.csproj') -c Release -r win-x64 --self-contained true -p:PublishSingleFile=false -o $hostStage
dotnet publish (Join-Path $repositoryRoot 'src\DeskBridge.App\DeskBridge.App.csproj') -c Release -r win-x64 --self-contained true -p:PublishSingleFile=false -o $appStage
